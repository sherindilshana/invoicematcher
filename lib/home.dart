...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String _statusMessage = 'Select Invoice and Purchase Order (PO) to start matching.';
  ...
  Future<void> _pickFile(bool isInvoice) async {
    ...
    if (result != null && result.files.first.bytes != null) {
      setState(() {
        ...
        setState(() => _statusMessage = 'Error reading file bytes. Try a different PDF.');
    }
  }
  ...
  Future<void> _runMatching() async {
    ...
    setState(() {
      _isLoading = true;
      _statusMessage = 'Processing documents with AI & Hybrid OCR/Table Extraction...'; 
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _matchResult = null;
          _statusMessage = 'Error: Server processing failed (Status ${response.statusCode}). Details: $responseBody';
        });
      }
    } catch (e) {
      setState(() {
        _matchResult = null;
        _statusMessage = 'Network Error: Could not connect to backend. Is the Python server running? Error: $e';
      });
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Widget _buildFileCard(PlatformFile? file, bool isInvoice) {
    return Card(
      elevation: 2,
      child: ListTile(
        leading: Icon(FontAwesomeIcons.filePdf, color: isInvoice ? Colors.red.shade700 : Colors.blue.shade700),
  ...
  Widget _buildDataSummary(Map<String, dynamic> data, String title, Color color) {
    // Determine box color for a cleaner palette. 
    final bool isInvoice = title.contains('INVOICE');
    final Color boxColor = isInvoice ? Colors.blueGrey.shade100.withOpacity(0.5) : color.withOpacity(0.1);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('ID: ${data['id'] ?? 'N/A'}', style: const TextStyle(fontSize: 14)), 
          Text('Vendor: ${data['vendor'] ?? 'N/A'}', style: const TextStyle(fontSize: 14)),
          Text('Total: \$${(data['total'] as num?)?.toStringAsFixed(2) ?? '0.00'}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
        ],
      ),
    );
  }
  ...
  Widget _buildResultCard(double screenWidth) {
    ...
    final invData = _matchResult!['invoice_data'] as Map<String, dynamic>? ?? {}; 
    ...
    if (isMobileLayout) {
      ...
      dataComparisonWidget = Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(child: _buildDataSummary(invData, 'INVOICE DATA (Extracted)', Colors.indigo)),
          const SizedBox(width: 20),
          Expanded(child: _buildDataSummary(poData, 'PO DATA (Extracted)', Colors.teal)),
        ],
      );
    }

    // Agent Summary/Explanation logic
    String agentSummary = explanations.isNotEmpty ? explanations.first.toString() : 'Processing result empty.';
    
    // Aggressively remove leading warning icon and trim.
    agentSummary = agentSummary.replaceFirst('⚠️', '').replaceFirst('!', '').replaceFirst('🚩', '').trimLeft();

    // Split the summary: primary status before the first dash, detail after.
    final int dashIndex = agentSummary.indexOf(' - ');
    final String primaryStatusText = dashIndex != -1 ? agentSummary.substring(0, dashIndex) : agentSummary;
    final String mainExplanationText = dashIndex != -1 ? agentSummary.substring(dashIndex + 3) : '';
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              crossAxisAlignment: CrossAxisAlignment.start, 
              children: [
                Icon(isApproved ? FontAwesomeIcons.circleCheck : FontAwesomeIcons.circleExclamation, color: color, size: 24),
            ...
            Padding(
              ...
              child: Text(
                ...
                style: mismatchSummaryTextStyle, // Applies clean, non-italic, no-underline, black text
              ),
            ),
            ...
            const Text('Detailed Findings (Agent Log):', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            const SizedBox(height: 8),
            // Checkmark/warning icon handling
            ...explanations.skip(1).map((exp) => Padding(
              padding: const EdgeInsets.only(bottom: 8.0, left: 10),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(exp.toString().contains('⚠️') ? '🔴' : '✅', style: const TextStyle(fontSize: 16)),
                  const SizedBox(width: 8),
                  Expanded(child: Text(exp.toString().replaceFirst('⚠️', '').trim())),
                ],
              ),
            )).toList(),
          ],
  ...
  Widget build(BuildContext context) {
    ...
    final double maxContentWidth = screenWidth > breakpoint 
      ? 850.0 
      : screenWidth * 0.95; 

    // Dynamic top padding: Large initially (40.0), reduced during/after matching (20.0).
    final double topPadding = (_matchResult == null && !_isLoading) ? 40.0 : 20.0;
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                // 2. RUN BUTTON
                ElevatedButton.icon(
                  onPressed: (_invoiceFile != null && _poFile != null && !_isLoading) ? _runMatching : null,
                  icon: _isLoading 
                      ? const SizedBox(width: 15, height: 15, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) 
                      : const Icon(FontAwesomeIcons.rocket),
                  label: Text(_isLoading ? 'MATCHING... PLEASE WAIT' : 'RUN AI MATCHING & REVIEW'),
