...
# --- NEW ROUTE: HEALTH CHECK (Fixes 404 Error) ---
@app.route('/', methods=['GET', 'HEAD'])
def health_check():
    """Returns a 200 OK status to satisfy automated health checks."""
    if client is None:
        # If the Gemini client failed to initialize, report a 503 (Service Unavailable)
        return jsonify({"status": "error", "message": "AI client unavailable"}), 503
